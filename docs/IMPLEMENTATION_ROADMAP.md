...
- Lần thử thứ năm (2026-09-23): server khởi động thành công (3 stage), nhưng cả 12 mẫu trả về event `error` rỗng. Đọc mã nguồn: `/v1/realtime?duplex=0` của vLLM-Omni là API chép lời của vLLM upstream, chỉ nhận `session.update {model}` / `input_audio_buffer.append` / `.commit`. Sửa ở v0.3.0: thêm kênh `transport: chat` qua `/v1/chat/completions` cho các tác vụ theo lượt; kênh realtime giữ lại cho Phase 6 và GPT-Realtime.
- **Kết quả checkpoint 2 cho MiniCPM-o 4.5 (2026-09-23, release v0.3.0):** 10/12 mẫu chạy được. Quan sát được: streaming audio (10/10 mẫu, 3–36 chunk mỗi câu), kênh text (10/10), huỷ phản hồi (2/2). Model trả lời bằng tiếng Nhật, ví dụ 「ご安心ください。ご用件についてお答えいたします。」. 2 mẫu tool call lỗi HTTP 400: `"auto" tool choice requires --enable-auto-tool-choice and --tool-call-parser to be set`. Sửa ở v0.3.1 bằng giao thức tool qua prompt (ARCHITECTURE §6.3).
- **MiniCPM-o 4.5 hoàn tất smoke (2026-09-24, v0.3.2): 12/12 mẫu.** Quan sát được: streaming audio, kênh text, huỷ phản hồi 2/2, và gọi tool qua prompt 1/2 mẫu (mẫu còn lại model trả lời thẳng, không gọi tool — hợp lệ vì nội dung FLEURS không phải yêu cầu nghiệp vụ).
- **Qwen3-Omni lần đầu (2026-09-24): không đủ bộ nhớ KV cache.** Đo được: weights thinker 59,26 GiB; với `gpu_memory_utilization: 0.78` chỉ còn 1,05 GiB cho KV cache, trong khi `max_model_len: 65536` cần 6,0 GiB. Sửa ở v0.3.3: đặt `max_model_len: 8192` cho stage 0 và nâng phần bộ nhớ của stage này.
