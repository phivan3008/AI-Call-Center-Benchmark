...
| Log server báo `ValueError: To serve at least one request with the model's max seq len ... KV cache` | Không đủ bộ nhớ GPU cho KV cache sau khi nạp weights. Giảm `max_model_len` của stage đó trong `configs/deploy/<model>_1gpu.yaml` (nhu cầu KV tỷ lệ thuận với độ dài ngữ cảnh), hoặc tăng `gpu_memory_utilization` của stage đó và giảm ở stage khác. Với Qwen3-Omni đã đặt sẵn `max_model_len: 8192` từ v0.3.3. |
| Mẫu tool call báo `http_400` với `"auto" tool choice requires --enable-auto-tool-choice` | Server chưa bật tool parser. Từ v0.3.1, các model cục bộ dùng giao thức tool qua prompt (`tool_protocol: prompted`) nên không cần cờ này. Nếu muốn thử function calling gốc, thêm `--enable-auto-tool-choice --tool-call-parser <parser>` vào `serve_args` và đổi `tool_protocol: native`. |
